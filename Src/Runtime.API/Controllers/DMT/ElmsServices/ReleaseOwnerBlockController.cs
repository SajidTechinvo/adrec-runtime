...
namespace Runtime.API.Controllers.DMT.ElmsServices
{
    ...
    public class ReleaseOwnerBlockController(IRedisCacheService redis, ILogger logger, IRestClientUnit rest) : ApiController(redis,logger)
    {
        ...
        [HttpPost("start")]
        public async Task<IActionResult> StartBackOfficeWorkflow(string args, StartReleaseOwnerBlock model)
        {
            var token = RequestHelper.GetAuthorizationToken(HttpContext.Request).Split(" ")[1];

            var cookies = await _redis.GetCacheValueAsync<List<Cookie>>(token);

            var result = await _rest.ReleaseOwnerBlock.StartWorkflow(cookies, args, model);
        ...
        //    var result = await _rest.ReleaseOwnerBlock.RegisterSubmit(cookies, args);
        ...
        //    var result = await _rest.ReleaseOwnerBlock.ApprovalApprove(cookies, args);
        ...
        //    var result = await _rest.ReleaseOwnerBlock.ApprovalReject(cookies, args, model);
        ...
        //    var result = await _rest.ReleaseOwnerBlock.GetStepInfo(cookies, args);
