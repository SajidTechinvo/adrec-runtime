...
{
    public interface IPublicHouseToResidentialLandClient
    {
        Task<ErrorOr<DmtResponseWrapper<PublicHouseToResidentialLandResponse>>> GetStepInfo(List<Cookie> cookies, string args);

        Task<ErrorOr<DmtResponseWrapper<WorkflowResponse>>> AuditWorkflow(List<Cookie> cookies, string args, BackOfficeModel model);
