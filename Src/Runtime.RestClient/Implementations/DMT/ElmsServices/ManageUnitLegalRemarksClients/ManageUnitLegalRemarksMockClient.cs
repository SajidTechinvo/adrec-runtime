...
{
    internal class ManageUnitLegalRemarksMockClient : IManageUnitLegalRemarksClient
    {
        ...
        private static async Task<string> GetFileContent(string fileName)
        {
            ...
            var filePath = Path.Combine(basePath, "Response", "ElmsServices", "ManageUnitLegalRemarks", fileName);
        ...
        public async Task<ErrorOr<DmtResponseWrapper<ManageUnitLegalRemarksResponse>>> GetStepInfo(List<Cookie> cookies, string args)
        {
            ...
            return JsonSerializer.Deserialize<DmtResponseWrapper<ManageUnitLegalRemarksResponse>>(content);
        }
